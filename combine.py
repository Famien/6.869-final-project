...
def closest_pixel(window, pixel):
    ...
    for i in range(len(window)):
        for j in range(len(window)):
            ...
            distance = abs(pixel-window_pixel)
...
def get_high_res_colored(lr_bw, lr_colored, hr_bw, window_size):
    ...
    print("lr bw: ", lr_bw.shape)
    print("lr colored: ", lr_colored.shape)
        ...
        for j in range(int(len(hr_bw[0])/window_size)):
            ...
            for k in range(window_size):
                for l in range(window_size):
                    print("window marked: ", window_marked[k,l])
                    print("lr colored; ", lr_colored[i,j])
